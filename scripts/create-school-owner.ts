...
async function run() {
  ...
  const { data: org } = await admin.from('organizations').select('id, name')
    .eq('slug', 'sydney-driving-academy').single();
  ...
  const { data: authUser, error: authErr } = await admin.auth.admin.createUser({
    ...
    user_metadata: { full_name: 'School Owner' },
  });
  ...
  const { data: existing } = await admin.from('organization_members')
    .select('id').eq('organization_id', org.id).eq('user_id', userId!).single();

  if (!existing) {
    const { error: memErr } = await admin.from('organization_members').insert({
      organization_id: org.id, user_id: userId!, role: 'school_owner', status: 'active',
    });
    if (memErr) console.error('❌ Member error:', memErr.message);
    else console.log('  ✅ Added as school_owner');
  } else {
